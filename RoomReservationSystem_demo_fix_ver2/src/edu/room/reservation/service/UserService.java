...
public class UserService {
    ...
    public UserService(UserRepository userRepo, ReservationRepository resRepo){
    ...
    public void delete(){
        try{
            ...
            User targetUser = userRepo.getMap().get(targetId);
            if(TroubleShooting.confirmDelete()){
                userRepo.remove(targetId);
                hasReservation(targetUser);
            }
    ...
    private void hasReservation(User targetUser){
        for(Reservation r : resRepo.getMap().values()){
            if(r.getOwner().equals(targetUser)) resRepo.remove(r.getId());
        }
    }
